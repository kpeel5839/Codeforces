...
ll get(ll v, ll y, ll x) {
  ...
  while (true) {
    ...
    //   previousDiff = diff;
    // } else {
    //   if (previousDiff - diff >= 2) {
    //     cout << previousDiff << " " << diff << "\n";
    //   }
    //   previousDiff = diff;
    // }

    ll limit = diff * y;
    ll nowCnt = (v - limit) / diff + 1ll;

    nowCnt = min(nowCnt, use);
    use -= nowCnt;
    v = v - (diff * nowCnt);
    cnt += nowCnt;
...
int main(void) {
  ...
  freopen("Educational_Codeforces_Round_184_(Rated_for_Div._2)/d.input.txt", "r", stdin);
  int T;
  cin >> T;
  while (T-- > 0) {
    solve();
