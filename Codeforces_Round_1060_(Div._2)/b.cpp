...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (arr[left] >= ma) {
      ll need = arr[left] - ma + 1ll;
      arr[left] -= need;
      answer += need;
    }
    if (right < n && arr[right] >= ma) {
      ll need = arr[right] - ma + 1ll;
      arr[right] -= need;
      answer += need;
    }
  }
...
int main(void) {
  ...
  freopen("Codeforces_Round_1060_(Div._2)/b.input.txt", "r", stdin);
  int T;
  cin >> T;
  while (T-- > 0) {
    solve();
