...
void solve() {
  ...
  unordered_map<int, vi> right;

  int a = 0, b = 0;
  for (int i = n - 1; 0 <= i; i--) {
    // left 기록하고, right 기록 하는데, 서로의 부호가 반대로 될 때까지 진행
    if (s[i] == 'a') {
      a++;
    } else {
      b++;
    }
    right[a - b].push_back(i);
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    vi& arr = right[need];
    if (arr.size() != 0) {
      int idx = bigger(i, arr);
      if (idx != -1) {
        answer = min(answer, arr[idx] - i - 1);
      }
    }
  }
...
int main(void) {
  ...
  freopen("Educational_Codeforces_Round_183_(Rated_for_Div._2)/c.input.txt", "r", stdin);
  int T;
  cin >> T;
  while (T-- > 0) {
    solve();
