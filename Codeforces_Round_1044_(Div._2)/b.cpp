...
void solve() {
  ...
  for (int i = 0; i < n / 2; i++) {
    int idx = i * 2;
    answer += max(newA[idx], newA[idx + 1]);
    ll minusValue = min(newA[idx], newA[idx + 1]);
    next.push_back(newA[idx] - minusValue);
  }
  if (isOdd) {
    answer += a[0];
  }
  // for (int i=0;i<(int)next.size()-1;i++) {
  //   answer+=max(next[i],next[i])
  // }
  cout << answer << "\n";
}
...
int main() {
  ...
  freopen("Codeforces_Round_1044_(Div._2)/b.input.txt", "r", stdin);
  int T;
  cin >> T;
  while (T-- > 0) {
    solve();
